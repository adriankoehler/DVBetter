...
watch(
  ...
  (newVal) => {
    ...
    const stationData = stationsJson.features.filter(
      (d) => d.properties.name == newVal
    );
    if (stationData.length > 1) {
      console.log(
        "multiple stations found for name, take first one",
        stationData[0]
      );
    } else if (stationData.length < 1) {
      console.log(
        "no geo data found for this station, map will only display default location"
      );
